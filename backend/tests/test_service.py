...
@pytest.mark.asyncio
async def test_upload_file_service_with_failure_in_metadata(database_fixture, file_service_with_metadata_failure, file_input):
...
@pytest.mark.asyncio
async def test_upload_file_service_with_failure_in_file(file_service_with_file_failure, file_input):
    with pytest.raises(Exception) as exc_info:
        await file_service_with_file_failure.upload_file_service(file_input)
    assert str(exc_info.value) == "File failure"

@pytest.mark.asyncio
async def test_list_files_metadata_service(populate_database_fixture, file_service_fixture, file_object):
